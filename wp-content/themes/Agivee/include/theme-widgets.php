...
if ( function_exists('register_sidebar') )
 ...
 register_sidebar(array(
  ...
  register_sidebar(array(
    ...
    'before_widget' => '<div class="maincontent"><div class="widgets">',
    ...
    if ($opt_childpage == "on") {
      ...
      if ($ID == 'homepagebox') { ?>
        ...
        <?php if ($image_thumbnail) : ?>
          <img src="<?php bloginfo('template_directory');?>/timthumb.php?src=<?php echo $image_thumbnail;?>&amp;h=80&amp;w=280&amp;zc=1" alt="" class="aligncenter" />      
        <?php else : ?>        
...
class Testimonial_Widget extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    if ($testititle == "") $testititle == __('Testimonial');
...
class SidePost_Widget extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    $check_opt = apply_filters('check_opt', $instance['check_opt']);
      ...
      <?php else : ?>
        <?php if (get_post_meta($post->ID,"thumbnail",true)) : ?>
        <img src="<?php bloginfo('template_directory');?>/timthumb.php?src=<?php echo get_post_meta($post->ID,"thumbnail",true);?>&amp;h=80&amp;w=280&amp;zc=1" alt="" class="aligncenter" />
        <?php endif; ?>
      <?php endif;?>
      </div>
      <?php }?>
...
class FeaturedProject_Widget extends WP_Widget {
  function FeaturedProject_Widget() {
    $widgets_opt = array('description'=>'Featured Project Agivee Theme Widget');
    parent::WP_Widget(false,$name= "Agivee - Featured Message ",$widgets_opt);
  }
  ...
  ?>
    <p><label for="featuredtitle">Title:
	 ...
	 <p><small>Please select your Portfolio category</small></p>
		<select  name="<?php echo $this->get_field_name('featuredcat'); ?>">  id="<?php echo $this->get_field_id('featuredcat'); ?>" >
			<?php foreach ($categories as $opt => $val) { ?>
		<option value="<?php echo $opt ;?>" <?php if ( $featuredcat  == $opt) { echo ' selected="selected" '; }?>><?php echo $val; ?></option>
		<?php } ?>
		</select>
		</label></p>	
    <p><label for="numtesti">Number to display:
  ...
  function widget( $args, $instance ) {
    ...
    if ($featuredtitle == "") $featuredtitle = __('Featured Project','agivee');
    if ($featurednum == "") $featurednum = 3;
    
    echo $before_widget;
    $titlefeatured = $before_title.$featuredtitle.$after_title;
    agivee_featuredproject($featuredcat,3,$titlefeatured);
    echo $after_widget;
  } 
}
...
class SocialLink_Widget extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    include(TEMPLATEPATH.'/widgets/social.php');
    echo $after_widget;
  } 
}
...
class Adbanner_Widget extends WP_Widget {
  function Adbanner_Widget() {
    ...
    parent::WP_Widget(false,$name= "Agivee - Banner Widget",$widgets_opt);
  }
  ...
  function widget( $args, $instance ) {
    ...
    if ($bannertitle == "") $bannertitle = __('Sponsor','agivee');
    
    echo $before_widget;
    $bannertitle = $before_title.$bannertitle.$after_title;
    include(TEMPLATEPATH.'/widgets/sponsor.php');
    echo $after_widget;
  } 
}
...
<?php echo __('Phone ','agivee');?>: <?php if ($contactphone) echo $contactphone; else echo "+62 1234 5678";?>,  <?php echo __('Email ','agivee');?>: <?php if ($contactemail) echo $contactemail; else echo "info@agivee.com";?><br />
