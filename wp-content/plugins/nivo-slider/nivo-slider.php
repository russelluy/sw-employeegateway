...
Description: The official WordPress plugin for the <a href="http://nivo.dev7studios.com">Nivo Slider</a> (Single License Version)
Version: 1.2.1
...
class WordpressNivoSlider {
    ...
    function init() {
        ...
        // Try and make sure cache is writable
        if( !is_writable(WP_PLUGIN_DIR .'/'. $this->plugin_folder .'/cache') ){
            @chmod(WP_PLUGIN_DIR .'/'. $this->plugin_folder .'/cache', 0777);
        }
    ...
    function admin_init() {
        ...
        add_meta_box( 'nivoslider_pro_box', __( 'Nivo Slider Pro', 'nivo-slider' ), array(&$this, 'meta_box_pro'), 'nivoslider', 'side' );
    }
    ...
    function admin_print_scripts() {
        ...
        if(isset($post->post_type) && $post->post_type == 'nivoslider'){
            wp_register_script( 'nivoslider-fileupload', WP_PLUGIN_URL .'/'. $this->plugin_folder .'/scripts/uploader/fileuploader.js' );
    ...
    function upload_image(){
        ...
        require_once(WP_PLUGIN_DIR .'/'. $this->plugin_folder .'/scripts/uploader/uploader.php');
        $response['error'] = false;
        $response['message'] = '';
    ...
    function meta_box_pro() {
        ?>
        <p>Want private priority support to help set up &amp; maintain your slider, unlimited WordPress 
        Plugin licenses and automatic updates? Get all this and much more with Nivo Slider Pro.</p>
        <p><a href="http://nivo.dev7studios.com/pro" target="_blank">Upgrade to Pro Now &rarr;</a></p>
        <?php 
    }
        ...
        if( $images ){
            ...
            foreach( $images as $image ){
                ...
                if(isset($options['disableCrop']) && $options['disableCrop'] == 'on'){ 
                    ...
                    $output .= '<img src="'. WP_PLUGIN_URL .'/'. $this->plugin_folder .'/timthumb.php?src='. $image['image_src'] .'&amp;h='. $options['dim_y'] .'&amp;w='. $options['dim_x'] .'&amp;zc=1&amp;q=100" ';
                }
                ...
                if(isset($options['controlNavThumbs']) && $options['controlNavThumbs'] == 'on'){
                    if(isset($options['disableCrop']) && $options['disableCrop'] == 'on'){ 
                        ...
                        $output .= 'rel="'. WP_PLUGIN_URL .'/'. $this->plugin_folder .'/timthumb.php?src='. $image['image_src'] .'&amp;h='. $options['thumbSizeHeight'] .'&amp;w='. $options['thumbSizeWidth'] .'&amp;zc=1&amp;q=90" ';
                    }
                }
