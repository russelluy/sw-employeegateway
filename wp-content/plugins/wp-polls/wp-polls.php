<?php
/*
Plugin Name: Safeway WP-Polls
Plugin URI: http://lesterchan.net/portfolio/programming/php/
Description: Adds an AJAX poll system to your WordPress blog. You can easily include a poll into your WordPress's blog post/page. WP-Polls is extremely customizable via templates and css styles and there are tons of options for you to choose to ensure that WP-Polls runs the way you wanted. It now supports multiple selection of answers.
Version: 2.61. Modification of original plugin was made for Safeway backstage gateway. Automatic update of this plugin has been disabled.
Author: Lester 'GaMerZ' Chan
Author URI: http://lesterchan.net
*/
define('DONOTCACHEDB', true);

if(!defined('add_filter'))
	require_once realpath(__DIR__.'/../../..').'/wp-load.php';
	
//if(!defined('add_shortcode'))
	//require_once realpath(__DIR__.'/../../..').'/wp-includes/shortcodes.php';
...
function check_voted_username($poll_id) {
	...
	$pollsip_user = $current_user->user_login;
	$log_expiry = intval(get_option('poll_cookielog_expiry'));
	$log_expiry_sql = '';
	if($log_expiry > 0) {
		$log_expiry_sql = 'AND ('.current_time('timestamp').'-(pollip_timestamp+0)) < '.$log_expiry;
	...
	$voted_aid_query = 'SELECT pollip_aid FROM ' . $wpdb->pollsip . ' WHERE pollip_qid = ' . $poll_id . ' AND pollip_user = \'' . $pollsip_user. '\' ' . $log_expiry_sql;
	$get_voted_aids = $wpdb->get_col($voted_aid_query);
	//echo $voted_aid_query;
	//print_r($get_voted_aids);	
	if($get_voted_aids) {
		return $get_voted_aids;
	} else {
		return 0;
...
function display_pollvote($poll_id, $display_loading = true) {
	...
	if($poll_question && $poll_answers) {
		...
		$temp_pollvote .= "<div style=\"float:left;\"><ul class=\"wp-polls-ul\">";
		$count = 0;
		foreach($poll_answers as $poll_answer) {
			...
			$count = $count+1;
			if($count > 8){
				$temp_pollvote .= "</ul></div><div style=\"float:left;margin-left:20px;\"><ul class=\"wp-polls-ul\">";
				$count = 0;
			}
		}
		$temp_pollvote .= "</ul></div>";
	...
	if(!empty($_POST['vote'])) {
		...
		if($poll_id > 0 && !empty($poll_aid_array) && check_allowtovote()) {
			...
			if($check_voted == 0) {
				...
				get_currentuserinfo();
				$pollip_user = 'guest';
				$pollip_user = $current_user->user_login ;
